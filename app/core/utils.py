"""log_audit, avaliar_alertas, criar_ou_atualizar_alerta, fechar_alerta_se_existir."""
...
from app.core.constants import NivelEscalonamento, ESCALONAMENTO_MINUTOS, TipoAlerta

logger = logging.getLogger(__name__)


def log_audit(
    db: Session, *, username: str, action: str,
    target: Optional[str] = None, details: Optional[dict] = None,
    ip_address: Optional[str] = None, request: Optional[Request] = None,
) -> None:
...
def avaliar_alertas(db: Session, sensor_id: int, leitura) -> None:
    ...
    if temp is not None:
        if equip.temp_max is not None and temp > equip.temp_max:
            criar_ou_atualizar_alerta(db, sensor_id=sensor_id, equipamento_id=equip.id,
...
def criar_ou_atualizar_alerta(
    db: Session, *, sensor_id: int, equipamento_id: int, tipo: str,
    valor_registrado: Optional[float] = None, valor_limite: Optional[float] = None,
) -> None:
    from app.core.models import Alerta
    existente = db.query(Alerta).filter(
        Alerta.sensor_id == sensor_id, Alerta.tipo == tipo, Alerta.fim_at.is_(None)
    ).first()
    if existente:
    ...
    db.add(Alerta(
        sensor_id=sensor_id, equipamento_id=equipamento_id, tipo=tipo,
        valor_registrado=valor_registrado, valor_limite=valor_limite,
        inicio_at=agora, fim_at=None,
        nivel_escalonamento=NivelEscalonamento.IMEDIATO.value,
        proxima_escalonamento_at=agora + timedelta(minutes=ESCALONAMENTO_MINUTOS[NivelEscalonamento.SEGUNDO]),
    ))
    db.flush()
